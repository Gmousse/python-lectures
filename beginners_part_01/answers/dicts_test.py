...
class Test_dicts(unittest.TestCase):
    ...
    def test_students_notes(self):
        ...
        def format_students(students, scores, student_to_find, student_new_score=None):
            ...
            students_count = len(students_scores)
            if student_new_score is not None:
                students_scores[student_to_find] = student_new_score
            score_to_find = students_scores[student_to_find]
            students_scores_list = list(students_scores.values())
            students_scores_list.sort()
            students_scores_list.reverse()
            best_worst_scores = [students_scores_list[0], students_scores_list[-1]]
            # STOP HERE
